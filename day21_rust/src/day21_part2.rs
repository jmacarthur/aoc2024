...
pub fn build_expansion_map() -> HashMap::<Vec<char>, Vec<char>>
{
    ...
    for dx in -2..=2 {
        for dy in -3..=3 {
            ...
            let mut r1: Vec<char> = horizontal_directions.chars().chain(vertical_directions.chars()).collect();
            r1.push('A');
            let r1expanded = find_directional_sequence(&r1);
            let mut r2: Vec<char> = vertical_directions.chars().chain(horizontal_directions.chars()).collect();
            r2.push('A');
...
pub fn expand(sequence: &[char], expansion_map: &HashMap<Vec<char>, Vec<char>>, result_map: &HashMap<(&[char], i32), usize>, 
    steps: i32) -> usize {
    if let Some(res) = result_map.get(&(sequence, steps)) {
        return *res;
    }
    ...
    if let Some(expanded) = expansion_map.get(sequence) {
        ...
        for s in splits {
            total_length += expand(s, expansion_map, result_map, steps-1);
        }
        total_length
    } else {
