...
fn evaluate_grid(
    ...
    loop {
        ...
        match grid.get(target_x, target_y) {
            ...
            Some(GridElement::Blocked) => direction = (direction + 3) % 4,
            None => {
...
fn main() -> std::io::Result<()> {
    ...
    for row in row_iterator {
        if !row.is_empty() {
            ...
            for (xpos, c) in row.chars().enumerate() {
                row_vector.push(match c {
                    ...
                    x => {
                        panic!(
                            "Unknown source character {} at row {}, column {}",
                            x, xpos, ypos
                        );
                    }
                });
    ...
    println!(
        "Part 2: Found {} positions which give infinite loops",
